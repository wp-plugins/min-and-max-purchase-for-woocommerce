...
class VTMAM_Apply_Rules{
  ...
  public function vtmam_minandmax_purchase_check() { 
    ...
    */  
    $sizeOf_vtmam_rules_set = sizeof($vtmam_rules_set);
    ...
    for($i=0; $i < $sizeOf_vtmam_rules_set; $i++) {                                                               
      if ( $vtmam_rules_set[$i]->rule_status == 'publish' ) {       
        for($k=0; $k < sizeof($vtmam_cart->cart_items); $k++) {                 
    ...
    */
    for($i=0; $i < $sizeOf_vtmam_rules_set; $i++) {         
        if ( $vtmam_rules_set[$i]->rule_status == 'publish' ) {  
          ...
          if ( sizeof($vtmam_rules_set[$i]->inpop_found_list) == 0 ) {
            ...
            $sizeOf_inpop_found_list = sizeof($vtmam_rules_set[$i]->inpop_found_list);
            /*
    ...
    /*   WITH BOTH MIN AND MAX RUNNING AT THE SAME TIME (POTENTIALLY), THIS CODE DOES NOT APPLY
    //****************************************************************************
    //   IF WE DON'T DO "apply multiple rules to product", rollout the multples   
    //****************************************************************************
    if ($vtmam_setup_options[apply_multiple_rules_to_product] == 'no' )  {
      for($k=0; $k < sizeof($vtmam_cart->cart_items); $k++) {             //$k = 'cart item'
         if ( sizeof($vtmam_cart->cart_items[$k]->product_participates_in_rule) > 1 ) {  
            //*****************************
            //remove product from **2ND** TO NTH rule, roll quantity and price out of totals for that rule
            //***************************** 
            for($r=1; $r < sizeof($vtmam_cart->cart_items[$k]->product_participates_in_rule); $r++) {   //$r = 'in rule'
              //disambiguation does not apply to products belonging to a varkgroup rule
              if (!$vtmam_cart->cart_items[$k]->product_participates_in_rule[$r]['inpop_selection'] == 'vargroup') {  //does not apply to vargroups!!
                  //use stored occurrences to establish addressability to this rule's info...
                  $rulesetLoc = $vtmam_cart->cart_items[$k]->product_participates_in_rule[$r]['ruleset_occurrence'];
                  $inpopLoc   = $vtmam_cart->cart_items[$k]->product_participates_in_rule[$r]['inpop_occurrence'];
                  //roll the product out of the rule totals, mark as 'no action required' for that rule!  
                  $vtmam_rules_set[$rulesetLoc]->inpop_qty_total   -= $vtmam_rules_set[$rulesetLoc]->inpop_found_list[$inpopLoc]['prod_qty'];
                  $vtmam_rules_set[$rulesetLoc]->inpop_total_price -= $vtmam_rules_set[$rulesetLoc]->inpop_found_list[$inpopLoc]['prod_total_price'];
                  $vtmam_rules_set[$rulesetLoc]->inpop_found_list[$inpopLoc]['prod_requires_action'] = 'no';
                  //if action amounts are 0, turn off action status for rule
                  if ( ($vtmam_rules_set[$rulesetLoc]->inpop_qty_total == 0) && ($vtmam_rules_set[$rulesetLoc]->inpop_total_price == 0) ) {
                    $vtmam_rules_set[$rulesetLoc]->rule_requires_cart_action = 'no'; 
                  }
                  unset ( $vtmam_cart->cart_items[$k]->product_participates_in_rule[$r] );//this array is used later in printing errors in table form 
              }
           }    
         }                                       
      }
                ...
                switch( $vtmam_rules_set[$i]->inpop_selection ) {
                 ...
                 default:  // 'groups' or 'cart' or 'vargroup'                                                 
                    ...
                    if ( $vtmam_rules_set[$i]->inpop_selection  == 'groups' ) {
                      ...
                      if ( ( sizeof($vtmam_rules_set[$i]->prodcat_in_checked) > 0 )  && ($vtmam_setup_options['show_prodcat_names_in_errmsg'] == 'yes' ) ) {  
                      ...
                      if ( ( sizeof($vtmam_rules_set[$i]->rulecat_in_checked) > 0 ) && ($vtmam_setup_options['show_rulecat_names_in_errmsg'] == 'yes' ) ) {  
  ...
  public function vtmam_create_table_error_message_mom ($msg_minandmax_label) { 
   ...
   public function vtmam_table_line ($i, $k){
     ...
     $message_line .= __('<span class="table-msg-line">', 'vtmam');
   ...
   public function vtmam_table_totals_line ($i){
     ...
     $message_totals .= __('<span class="table-totals-line">', 'vtmam');
   ...
   public function vtmam_table_titles() {
     global $vtmam_info;
     $message_title;       
          $message_title  .= __('<span class="table-titles">', 'vtmam');
...
*/           
   public function vtmam_table_text_line ($i){
      ...
      $message_text .= __('<span class="table-error-msg"><span class="bold-this color-grp', 'vtmam');
...
public function vtmam_product_is_in_inpop_group ($i, $k) { 
    ...
    public function vtmam_list_out_product_names($i) {
      $prodnames;
    	...
    	for($p=0; $p < sizeof($vtmam_rules_set[$i]->errProds_names); $p++) {
          $prodnames .= __(' "', 'vtmam');
